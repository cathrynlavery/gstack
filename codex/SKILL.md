...
```bash
CODEX_SYNC_STAMP="${XDG_CACHE_HOME:-$HOME/.cache}/gstack/codex-npm-latest.stamp"
mkdir -p "$(dirname "$CODEX_SYNC_STAMP")"
if [ ! -f "$CODEX_SYNC_STAMP" ] || ! find "$CODEX_SYNC_STAMP" -mmin -1440 | grep -q . 2>/dev/null; then
  npm install -g @openai/codex@latest >/tmp/gstack-codex-npm-install.log 2>&1 && touch "$CODEX_SYNC_STAMP" || true
fi
CODEX_BIN=$(command -v codex || echo "")
[ -z "$CODEX_BIN" ] && echo "NOT_FOUND" || { echo "FOUND: $CODEX_BIN"; codex --version 2>/dev/null || true; }
```

If `NOT_FOUND`: stop and tell the user:
"Codex CLI not found. Install it: `npm install -g @openai/codex@latest` or see https://github.com/openai/codex"
...
Review the changes on this branch against the base branch <base>. Run git diff origin/<base>...HEAD 2>/dev/null || git diff <base>...HEAD to see the diff and review only those changes." -c 'model="gpt-5.5"' -c 'model_reasoning_effort="high"' --enable web_search_cached < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
...
_gstack_codex_timeout_wrapper 330 codex exec -m gpt-5.5 -s read-only "$(cat "$_PROMPT_FILE")" -c 'model_reasoning_effort="high"' --enable web_search_cached < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
rm -f "$_PROMPT_FILE"
if [ "$_CODEX_EXIT" = "124" ]; then
  _gstack_codex_log_event "codex_timeout" "330"
...
_gstack_codex_timeout_wrapper 600 codex exec -m gpt-5.5 "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
import sys, json
...
_gstack_codex_timeout_wrapper 600 codex exec -m gpt-5.5 "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
import sys, json
for line in sys.stdin:
    line = line.strip()
    if not line: continue
...
# Fix 1: wrap with timeout (gtimeout/timeout fallback chain via probe helper)
_gstack_codex_timeout_wrapper 600 codex exec resume <session-id> -m gpt-5.5 "<prompt>" -c 'sandbox_mode="read-only"' -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
...
**Model:** gstack pins Codex to `gpt-5.5` by default so `/codex` does not drift on
stale local defaults. Step 0.4 also refreshes the Codex CLI with
`npm install -g @openai/codex@latest` on a 24-hour cache, so the wrapper stays on the
latest released CLI without making the user remember to update it manually.
If the user wants a specific model, pass `-m` through to codex and let that override
the default.
...
If the user specifies a model (e.g., `/codex review -m gpt-5.5`
or `/codex challenge -m gpt-5.4`), pass the `-m` flag through to codex.
